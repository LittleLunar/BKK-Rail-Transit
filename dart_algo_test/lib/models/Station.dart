  ...
  static Future<Map<String, Station>> loadFromJSON(String path) async {
    ...
    for (String stationType in decoded.keys) {
      for (String stationName in decoded[stationType].keys) {
        returner[stationName] = Station(
            name: stationName,
            lineNumber: decoded[stationType][stationName]["lineNumber"]);
    ...
    for (String stationType in decoded.keys) {
      for (String stationName in decoded[stationType].keys) {
        for (String adjacencyName in decoded[stationType][stationName]
            ["adjacencies"]) {
          returner[stationName]?.addAdjacency(returner[adjacencyName]);
        }
      }
    ...
    if (identical(returner["Khu Khot"]?.getAdjacencies["Yaek Kor Por Aor"],
        returner["Yaek Kor Por Aor"])) {
      print("Same!!!!!!!!!!!!!!!!!!!!!!\n");
    }
